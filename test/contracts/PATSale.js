...
contract('PATSale', async function([dump, manager, wallet, investor, purchaser, visitor]) {
    ...
    describe('time dependant', function() {
        ...
        before(async function () {
          ...
          this.token = await PATToken.new(this.registered_user.address, id, managers, name, symbol, fixed_linkDoc, fixed_hashDoc, var_linkDoc, var_hashDoc);
          this.crowdsale = await PATSale.new(this.registered_user.address, this.raxToken.address,
                                            this.token.address, this.manageListingFee.address, this.manageReserveFunds.address, this.startTime, this.endTime, wallet,
                                            minCap, maxCap, ethPATRate, ethRAXRate, listingFeeRate, reserveFundRate);
    ...
    describe('during sale', function() {
        ...
        describe('high-level purchase (fallback)', function () {
          ...
          it('should fulfille buy PAT token use RAX', async function() {
        ...
        describe('low-level purchase (buyTokensUsingEther)', function () {
          beforeEach(async function() {
            this.tx_result = await this.crowdsale.buyTokensUsingEther(investor, {value: value, from: purchaser});
            await advanceBlock();
          });
        ...
        describe('token cap reached', function () {
          ...
          it('should refund when crowdsale unsuccessful', async function() {
            var amountETH = ether(1);
            await this.crowdsale.buyTokensUsingEther(purchaser, {value: amountETH, from: purchaser});
          ...
          it('should refund RAX when crowdsale unsuccessful', async function() {
            var amountRAX = new BigNumber(75000*10*(10**18));
            await this.raxToken.mint(purchaser, amountRAX, {from: dump}).should.be.fulfilled;
            await this.raxToken.mint(purchaser2, amountRAX, {from: dump}).should.be.fulfilled;
            await this.raxToken.approve(this.crowdsale.address, amountRAX, {from: purchaser}).should.be.fulfilled;
            await this.raxToken.approve(this.crowdsale.address, amountRAX, {from: purchaser2}).should.be.fulfilled;
            await this.crowdsale.buyTokensUsingRaxApprove(purchaser, {from: purchaser});
            ...
            await this.crowdsale.finalize({from: dump}).should.be.fulfilled;
            await this.crowdsale.claimRefund({from: purchaser2}).should.be.fulfilled;
            var patTokensAfter = await this.raxToken.balanceOf(purchaser2);
            patTokensAfter.should.be.bignumber.equal(raxTokensBefore.plus(amountRAX));
          });
        ...
        describe('enable and disable Token', function () {
          let purchaser2 = "0x28a8746e75304c0780e011bed21c72cd78cd535e";
          const tokens = n => new BigNumber(n).mul(10**18);
          const valueETH = ether(10);
          it('should fulfille buy tokens when enable Token', async function() {
            await this.crowdsale.buyTokensUsingEther(purchaser2, {value: valueETH, from: purchaser2}).should.be.fulfilled;
          });

          it('should reject buy tokens when enable Token', async function() {
            await this.token.disableToken({from: manager}).should.be.fulfilled;
            await this.crowdsale.buyTokensUsingEther(purchaser2, {value: valueETH, from: purchaser2}).should.be.rejected;
          });

          it('should can enable Token after disable Token', async function() {
            await this.token.disableToken({from: manager}).should.be.fulfilled;
            await this.token.enableToken({from: manager}).should.be.fulfilled;
            await this.crowdsale.buyTokensUsingEther(purchaser2, {value: valueETH, from: purchaser2}).should.be.fulfilled;
          });
        });
    });
